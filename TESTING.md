...
本ドキュメントは、JUnit 5 / Mockitoを使用した単体テストを中心に記録しています。

これとは別に、実際の画面操作を中心としたブラックボックステストを実施しています。

ブラックボックステストでは以下を確認しています。
...
ブラックボックステストでは、実施可能なケースはすべてOK、NGは0件でした。

`BB-UED-004`（最後のADMINをUSERへ変更）と
`BB-UED-008`（最後のADMINを削除）は、
最後のADMINがログインユーザー本人となり、自己操作禁止の条件と重複するため、
画面操作では最後のADMIN保護だけを単独検証できず保留としています。

ただし、最後のADMIN保護そのものは本ドキュメントに記載している
`AppUserService`のJUnitテストで確認しています。

また、ブラックボックステスト中に会員詳細画面へのタグ表示不足を発見し、
実装修正後にテストケースを追加して再テストしています。
...
JUnit / Mockito
    ↓
内部ロジック・分岐・Repository呼び出しを検証

ブラックボックステスト
    ↓
利用者視点で画面・入力・権限・画面遷移を検証
```

という2つの観点からテストを実施しています。
